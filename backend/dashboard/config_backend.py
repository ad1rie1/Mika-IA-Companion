...
user model — the accounts that log into the frontend and the dashboard.
...
leave the dashboard to hand a second login to someone.
...
Three rules the generic backend can't express, all enforced here:
  ...
  - **No lockout.** A write that would leave zero active superusers, or
    ...
    demoting are all the same mistake from the dashboard's point of view:
    the next reload of ``/dashboard/`` under ``DASHBOARD_REQUIRE_AUTH``
...
class UserAccountBackend:
    ...
    def _guard_no_lockout(self, obj, *, is_active: bool, is_staff: bool, is_superuser: bool,
        ...
        if after_staff == 0 and before_staff > 0:
            raise ValidationError(
                "Impossible : ce serait le dernier compte avec accès au dashboard. "
                "Donne l'accès dashboard à un autre compte d'abord."
            )
