...
`ModulePanel` handlers return **typed blocks** (`Table` / `Fields` / `Stats` / `Note` / `Prose` / `Template`) built from **typed cells** (`text`/`badge`/`emotion`/`meter`/`link`/`mono`/`num`/`bool`). A module declares an *intent*; it never emits markup, so the stored-XSS class is **removed rather than filtered**. `Template` is the honest escape hatch: the module ships its own Django template — code it owns, still autoescaped. Handlers may be sync or async. Legacy `get_views()` modules are adapted automatically (`_adapt_legacy_view`), including the 0-based → 1-based page conversion, so email and forge kept working untouched.
...
Tests: `test_gestion_systeme.py` (81) — every nav destination and tab derived from `nav.py` (a tab added without a template turns it red), every config section and record-list form, pagination elision and out-of-range clamping, closed-list guards on every URL value that reaches the ORM, `emotion_var` refusing anything outside the 29 names, secrets absent from rendered HTML, the checkbox-marker semantics (against a synthetic bool ConfigItem — the registry declares none, so a registry-derived test would silently skip), a legacy `html` payload rendering nothing, a raising panel degrading to an error block, module spaces surviving a stopped module, the auth gate's four cases, and the two trap guards above. Verified non-vacuous by mutation on ten points.
