...
**CSRF is enforced** (was the known remaining gap). `CsrfViewMiddleware` is installed, **no endpoint is `@csrf_exempt` any more** (24 decorators removed — a middleware every view opts out of protects nothing), and a test asserts the string never comes back. This only started mattering when sessions gained authority: while every endpoint was anonymous, a forged request bought an attacker nothing they couldn't do by calling the API directly; with a logged-in owner session, `PATCH /dashboard/api/config/values` rewrites provider API keys.

- Token plumbing: `/auth/whoami` carries `@ensure_csrf_cookie` (it is the client's *first* call, so the cookie exists before any POST); the SPA reads it in `postJson()`; the dashboard patches `window.fetch` **once** in `app.js` to attach `X-CSRFToken` to every mutating **same-origin** request — rather than editing ~16 scattered call sites and requiring every future one, including module-supplied views, to remember.
...
- **Every write delegates to `identity_resolver`** (accept / reject / bind / evidence / revoke). No certainty arithmetic and no `Identity.entity` assignment lives here: a second implementation is how the dashboard and the prompt start disagreeing about who someone is. Two deliberate differences from the tool path: a refused write is a 4xx rather than `{"ok": false}` with a 200 (the resolver is lenient because it runs on LLM-supplied arguments; a form is not), and an unknown evidence `kind` is refused rather than scored as 0 — from a fixed dropdown that is a bug, and a write that silently changes nothing is worse than an error.
- **Ephemeral `anon_*` handles are hidden by default** and the stat cards count the same scope the table shows (86 sockets against 9 real handles on the dev DB — a card reading "95" above a table of 9 reads as a broken filter). An identity with *no* handle is still listed: an orphan row is a fact about the database.
...
- Pending claims badge the sidebar via `overview.counts.identity` — a claim is deliberately not scored until someone decides, so it waits on a human the way a pending project action does.
- Tests: `test_dashboard_identity.py` (32) — effective-vs-stored certainty, `may_disclose` agreeing with `trust.may_disclose_private_context` across four channel/certainty pairs, public never clearing the bar, accept landing exactly on 0.70 with corroboration, double-resolution refused loudly, revoke unbinding while keeping the ledger, and the policy payload read off the module. Verified non-vacuous by mutation: dropping the channel floor turns the file red.
...
- **Account tests** (`test_dashboard_accounts.py`, 33): a created row is a real account that authenticates, the password hash never appears in a read payload (nor with `decrypt_secrets=True`, nor in the audit log), weak passwords refused at creation *and* on update, blank input keeps the current password, the four lockout guards (delete / deactivate / demote / drop dashboard access on the last admin), an inactive second admin not counting, and a database with no admin at all staying writable. Verified non-vacuous by mutation: an "after ≥ 1" guard instead of before-vs-after turns three tests red.
